...
# ---------- MSRP REFERENCE ----------
# These are practical reference prices for common sealed formats.
# If a product is not easy to identify confidently, the bot shows MSRP Unknown.
...
def estimate_msrp(title):
    ...
    if (
        ("booster display" in t or "booster box" in t)
        and ("36 pack" in t or "36-pack" in t or "36 packs" in t)
    ):
        return 161.64
    ...
    if "3 pack blister" in t or "3-pack blister" in t or "three pack blister" in t:
        return 14.99
...
def msrp_comparison(price_text, title):
    ...
    difference = round(price - msrp, 2)

    if abs(difference) < 0.01:
        return msrp, "🟡 AT MSRP"

    if difference < 0:
        return msrp, f"🟢 ${abs(difference):.2f} BELOW MSRP"

    return msrp, f"🔴 ${difference:.2f} ABOVE MSRP"
    ...
    for tag in soup.find_all(
        ...
        for item in stack:
            if (
                ...
                and isinstance(item.get("@graph"), list)
            ):
...
def availability_from_jsonld(product):
    ...
    if "preorder" in availability or "pre-order" in availability:
...
def third_party_seller(store, text, product=None):
    ...
    if isinstance(product, dict):
        ...
        if isinstance(seller, str) and seller.strip():
            seller_lower = seller.strip().lower()

            if not any(
                expected in seller_lower
    ...
    for pattern in patterns:
        ...
        seller = re.split(
            r"\||returns|shipping|delivery",
            seller,
        )[0].strip()

        if seller and not any(
            expected in seller
            for expected in direct
        ):
...
def special_tag(title):
    ...
    if "30th" in t or "30th anniversary" in t:
...
def send_discord(title, lines, url=None, color=3447003):
    ...
    result = subprocess.run(
        [
            ...
            "-H",
            "User-Agent: PokemonTCGDrops/1.0",
            "-d",
            payload,
            WEBHOOK,
        ],
...
def inspect_product(
    ...
    if product:
        status, price = availability_from_jsonld(
            product
        )
    ...
    is_third_party, seller = third_party_seller(
        store,
        body,
        product,
    )
...
def alert_for_item(
    ...
    msrp, value_label = msrp_comparison(
        item.get("price"),
        item["title"],
    )
    ...
    if kind == "restock":
        title = (
            f"🚨 RESTOCK — {store['name']}"
        )
        color = 15158332

    elif kind == "preorder":
        title = (
            f"🟣 PREORDER OPEN — {store['name']}"
        )
        color = 10181046

    else:
        title = (
            f"🆕 NEW LISTING — {store['name']}"
        )
        color = 3447003
    ...
    with httpx.Client() as client:
        for store in STORES:
            ...
            for url, old in known[:5]:
                ...
                if (
                    not initialized
                    or fresh.get("third_party")
                ):
                    continue

                if (
                    old_status
                    in ("out_of_stock", "unknown")
                    and fresh["status"]
                    in ("in_stock", "preorder")
                ):
                    kind = (
                        "preorder"
                        if fresh["status"] == "preorder"
                        else "restock"
                    )
